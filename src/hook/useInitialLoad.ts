...
/**
 * The initial site-load gate — ONCE per app mount, never per navigation.
 *
 * This hook is deliberately app-level: App calls it once and renders the
 * loading overlay above whatever the "page" is. When the project grows a
 * router (Home → Projects → Home), the router outlet mounts BELOW this gate,
 ...
 *  video posters (videos themselves stream in over their poster after reveal,
...
export interface InitialLoadState {
  /** True until the boot gate opens; never returns to true afterwards. */
  isLoading: boolean;
  /** Eased progress 0–100, kept in sync with the progress bar and % text. */
  progress: number;
}
